...
const DEFAULTS = { admin: true, technician: true, sales: false, reception: false, user: false }
...
export function useTrackingConfig() {
  ...
  function isRoleEnabled(role) {
    ...
    if (trackingRoles.value === null) return role === 'admin' || role === 'technician'
    return !!trackingRoles.value[role]
  }

  function enabledRoles() {
    if (!trackingRoles.value) return ['admin', 'technician']
