<template>
  <div class="modernisation-view">
    <!-- Page Header -->
    <div class="page-header">
      <div>
        <h1 class="page-title">
          <Wrench :size="22" style="display:inline;margin-right:8px;vertical-align:-4px;" />
          Modernisation Activities
        </h1>
        <p class="page-sub">Breathing new life into ageing elevator systems.</p>
      </div>
    ...
    <div class="table-container">
      ...
      <div v-else-if="!filteredActivities.length" class="empty-state">
        <Wrench :size="40" style="opacity:.3;margin-bottom:12px;" />
        <p>No modernisation activities found.</p>
      </div>
          ...
          <tr v-for="a in filteredActivities" :key="a.id" @click="openView(a)" style="cursor:pointer;">
            ...
            <td>
              <div style="display:flex;align-items:center;gap:8px;">
                <div style="flex:1;height:6px;background:rgba(255,255,255,0.08);border-radius:3px;min-width:60px;">
                  <div :style="`width:${a.progress || 0}%;height:100%;background:#8b5cf6;border-radius:3px;`"></div>
                </div>
            ...
            <td>
              <div style="display:flex;gap:6px;flex-wrap:nowrap;" @click.stop>
                ...
                <button class="btn-secondary btn-sm" @click="exportCertificatePDF(a)" title="Download Certificate">
                  <FileText :size="12" />
                </button>
    ...
    <AppModal
      v-model="showModal"
      :title="editingId ? 'Edit Activity' : 'Log Modernisation Activity'"
      :subtitle="editingId ? 'Update activity record' : 'Record modernisation progress'"
      width="720px"
    >
      <form @submit.prevent="saveActivity" class="form-grid">
        ...
        <div class="form-group form-full" style="margin-bottom:4px;">
          <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;background:rgba(139,92,246,0.06);border:1px solid rgba(139,92,246,0.18);border-radius:12px;">
            <div style="display:flex;align-items:center;gap:10px;">
              <div style="width:36px;height:36px;background:rgba(139,92,246,0.12);border-radius:9px;display:flex;align-items:center;justify-content:center;color:#c084fc;">
                <FolderOpen :size="16" />
              </div>
              <div>
            ...
            <button
              ...
              :style="`width:44px;height:24px;border-radius:99px;border:none;cursor:pointer;transition:all .2s;background:${useProject ? '#8b5cf6' : 'rgba(255,255,255,0.12)'};position:relative;`"
            >
        ...
        <div v-if="useProject" class="form-group form-full">
          ...
          <div style="position:relative;">
            ...
            <div v-if="showProjectDropdown && filteredProjectOptions.length" class="proj-dd" style="position:absolute;top:100%;left:0;right:0;border-radius:8px;z-index:100;max-height:180px;overflow-y:auto;">
              <div
                ...
                :style="form.projectId === p.id ? 'background:rgba(139,92,246,0.15);' : ''"
              >
          ...
          <div v-if="form.projectId && form.projectAddress" style="margin-top:8px;display:flex;gap:10px;padding:10px 14px;background:rgba(139,92,246,0.06);border:1px solid rgba(139,92,246,0.15);border-radius:10px;font-size:12px;">
            <div style="color:#c084fc;flex-shrink:0;">📍</div>
          ...
          <div v-if="form.projectId && selectedProject && selectedProject.clients && selectedProject.clients.length > 0" class="form-group" style="margin-top:10px;">
            ...
            <div style="display:flex;flex-direction:column;gap:6px;padding:10px;background:rgba(139,92,246,0.05);border:1px solid rgba(139,92,246,0.15);border-radius:10px;">
        ...
        <div class="form-group form-full">
          ...
          <input v-model.number="form.progress" type="range" min="0" max="100" step="5" style="width:100%;accent-color:#8b5cf6;" />
        ...
        <!-- Completion Photos -->
        <div class="form-group form-full">
          <div style="padding:14px 16px;background:rgba(139,92,246,0.05);border:1px solid rgba(139,92,246,0.15);border-radius:10px;">
          ...
          <div class="form-group">
            ...
            <div style="display:flex;align-items:center;gap:8px;margin-top:4px;">
              <div style="flex:1;height:8px;background:rgba(255,255,255,0.08);border-radius:4px;">
                <div :style="`width:${viewTarget.progress || 0}%;height:100%;background:#8b5cf6;border-radius:4px;`"></div>
              </div>
    ...
    <AppModal v-model="showCompleteModal" title="Mark as Completed" subtitle="Capture completion signature" width="600px">
      <div style="display:flex;flex-direction:column;gap:16px;" v-if="completeTarget">
        <div style="padding:12px 14px;background:rgba(34,197,94,0.06);border:1px solid rgba(34,197,94,0.2);border-radius:10px;font-size:13px;color:#4ade80;">
          Marking modernisation at <strong>{{ projectName(completeTarget.projectId) }}</strong> as <strong>Completed</strong>
        </div>
        <div class="form-grid">
          <div class="form-group form-full">
            <label class="label">Completion Remarks</label>
            <textarea v-model="completeForm.resolutionNote" class="input" rows="2" placeholder="What was completed…" style="resize:vertical;"></textarea>
          </div>
          <div class="form-group">
        ...
        <div style="padding:12px 14px;background:rgba(139,92,246,0.06);border:1px solid rgba(139,92,246,0.15);border-radius:10px;">
    ...
    <ConfirmDialog
      ...
      message="Delete this modernisation activity record? This cannot be undone."
      confirmLabel="Delete"
      :danger="true"
    ...
    <ActivityTimeline
      ...
      :collection="Collections.MODERNISATION"
      :title="`Modernisation — ${projectName(timelineRecord?.projectId)}`"
      :record-summary="`${timelineRecord?.liftType || ''} · Tech: ${timelineRecord?.technician || '—'} · ${timelineRecord?.progress || 0}% complete`"
    />

    <!-- Import Modal -->
    <AppModal v-model="showImportModal" title="Import Modernisation Activities from Excel" width="640px">
      <div style="margin-bottom:16px;">
        ...
        <div v-if="importPreview.length" class="glass" style="padding:12px;border-radius:12px;">
          ...
          <div style="max-height:120px;overflow-y:auto;font-size:11px;color:var(--ct-muted);">
            <div v-for="(r, i) in importPreview.slice(0,5)" :key="i" style="padding:2px 0;">{{ r.liftNumber }} — {{ r.technician }} ({{ r.activityDate }})</div>
  ...
  <!-- Log Payment Modal -->
  <AppModal v-model="payModal" title="Log Payment Received" subtitle="Record a payment against this modernisation activity" width="480px">
    <div v-if="payTarget" style="display:flex;flex-direction:column;gap:16px;">
      <div style="padding:10px 14px;background:rgba(16,185,129,0.06);border:1px solid rgba(16,185,129,0.18);border-radius:10px;font-size:13px;color:#4ade80;">
        Logging payment for <strong>{{ payTarget.projectClient || projectName(payTarget.projectId) || payTarget.id }}</strong>
      </div>
      <div class="form-grid">
        <div class="form-group">
...
import {
  Wrench, Plus, Search, Pencil, Trash2, Eye,
...
const workDoneOptions = [
  'Controller Upgrade', 'Drive Upgrade', 'Cabin Renovation',
  'Door System', 'Lighting', 'Safety Devices', 'Cabling', 'Testing',
]
...
async function saveCompletion() {
  ...
  saving.value = true
  try {
    await update(Collections.MODERNISATION, completeTarget.value.id, {
      status: 'completed',
      progress: 100,
    ...
    await logTimeline(Collections.MODERNISATION, completeTarget.value.id, 'status_change', `Marked as Completed by ${authStore.user?.fullName || 'User'}`)
    ui.success('Modernisation marked as completed')
...
async function loadData() {
  ...
  try {
    ...
    ui.error('Failed to load modernisation activities')
  } finally {
    loading.value = false
  }
}
...
watch([() => ui.pendingAutoOpen, activities], () => {
  ...
  if (!pending || pending.collection !== Collections.MODERNISATION) return
...
async function saveActivity() {
  ...
  try {
    if (editingId.value) {
      await update(Collections.MODERNISATION, editingId.value, { ...form.value })
      activity.log({ action: 'updated', module: 'modernisation', tab: 'Modernisation', summary: `Updated modernisation activity for ${form.value.clientName || form.value.projectId || '—'}`, details: { clientName: form.value.clientName, liftType: form.value.liftType, status: form.value.status, progress: form.value.progress } })
      ui.success('Activity updated')
      if (editingStatus && editingStatus !== form.value.status) {
        await logTimeline(Collections.MODERNISATION, editingId.value, 'status_change',
          `Status changed: ${editingStatus} → ${form.value.status}`)
      } else {
        await logTimeline(Collections.MODERNISATION, editingId.value, 'updated',
          `Progress updated to ${form.value.progress || 0}%`)
      }
    } else {
      const newId = await create(Collections.MODERNISATION, { ...form.value })
      activity.log({ action: 'created', module: 'modernisation', tab: 'Modernisation', summary: `Logged modernisation activity for ${form.value.clientName || form.value.projectId || '—'}`, details: { clientName: form.value.clientName, liftType: form.value.liftType, activityDate: form.value.activityDate } })
      ui.success('Activity logged')
      await logTimeline(Collections.MODERNISATION, newId, 'created',
        `Modernisation activity created — ${form.value.liftType || ''} by ${form.value.technician || '—'}`)
    }
...
async function deleteActivity() {
  ...
  try {
    await remove(Collections.MODERNISATION, deleteTarget.value.id)
    activity.log({ action: 'deleted', module: 'modernisation', tab: 'Modernisation', summary: `Deleted modernisation activity for ${deleteTarget.value.clientName || deleteTarget.value.id}`, details: { clientName: deleteTarget.value.clientName, liftType: deleteTarget.value.liftType } })
...
async function savePayment() {
  ...
  try {
    ...
    await update(Collections.MODERNISATION, payTarget.value.id, {
...
async function exportCertificatePDF(a) {
  try {
    ...
    let y = await _drawReceiptHeader(doc, 'Modernisation Completion Certificate', `Certificate ID: ${a.id?.slice(0, 12) || '—'}`)
    ...
    const stmt = 'This certifies that the elevator modernisation work has been successfully completed at the site below, in compliance with all applicable standards and client requirements.'
    const stmtLines = doc.splitTextToSize(stmt, W - M * 2 - 6)
    doc.text(stmtLines, M + 3, y + 6)
    y += 22

    // ── Project Details ──────────────────────────────────────────────────────
    y = _sectionLabel(doc, 'Project Details', y, purple)
    y = _infoGrid(doc, [
    ...
    await savePDF(doc, `Modernisation-Certificate-${(proj?.projectName || a.id || 'cert').replace(/\s+/g, '_')}.pdf`, ui)
...
function triggerExport(type) {
  showExportDialog({
    ...
    filename: 'modernisation-activities',
    accent: [139, 92, 246],
    ui,
    pdfRowMapper: a => [
...
const importColumns = [
  { key: 'liftNumber', example: 'Lift-01', required: true },
  { key: 'technician', example: 'Ravi Kumar', required: true },
  { key: 'activityDate', example: '2024-03-15', required: true },
  { key: 'status', example: 'in-progress', required: false },
  { key: 'progress', example: '50', required: false },
  { key: 'remarks', example: 'Controller upgraded', required: false },
  { key: 'nextStepDate', example: '2024-03-22', required: false },
]
...
function onImportFile(e) {
  ...
  reader.onload = (ev) => {
    try {
      ...
      let skipped = 0
      rows.forEach(row => {
        const liftNumber = String(row.liftNumber || row['Lift Number'] || row['lift_number'] || '').trim()
        const technician = String(row.technician || row['Technician'] || '').trim()
        const activityDate = String(row.activityDate || row['Activity Date'] || row['activity_date'] || '').trim()
        if (!liftNumber || !technician || !activityDate) { skipped++; return }
        valid.push({
          liftNumber, technician, activityDate,
          status: String(row.status || 'in-progress').trim(),
          progress: Math.min(100, Math.max(0, Number(row.progress || 0))),
          remarks: String(row.remarks || '').trim(),
          nextStepDate: String(row.nextStepDate || row['Next Step Date'] || '').trim(),
          workDone: [],
          photos: [],
        })
      })
...
async function doImport() {
  ...
  for (const row of importPreview.value) {
    try {
      await create(Collections.MODERNISATION, { ...row, createdAt: new Date() })
      success++
    } catch {}
  }
  ui.success(`Imported ${success} activity record(s).`)
  closeImport()
  await loadData()
...
watch(() => ui.scheduleContext, (ctx) => {
  if (!ctx || ctx.tab !== 'modernisation') return
  editingId.value = null
  form.value = defaultForm()
