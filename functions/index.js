...
// ─────────────────────────────────────────────────────────────────────────────
// OTP — SEND (email + SMS simultaneously)
// ─────────────────────────────────────────────────────────────────────────────
...
async function sendOtpEmail(toEmail, otp) {
  ...
  await transporter.sendMail({
    from: `"Avant Elevators" <${process.env.ZOHO_EMAIL}>`,
    to: toEmail,
    subject: `${otp} — Your Avant Admin OTP`,
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:auto;padding:32px;background:#f8fafc;border-radius:12px;">
        ...
        <p style="color:#64748b;margin:0 0 24px;">Use the code below to complete your Avant admin login. Valid for <strong>5 minutes</strong>.</p>
        <div style="background:#1e1b33;border-radius:12px;padding:24px;text-align:center;letter-spacing:12px;font-size:36px;font-weight:700;color:#a5b4fc;">${otp}</div>
        <p style="color:#94a3b8;font-size:12px;margin-top:20px;">Do not share this OTP with anyone. If you did not attempt login, please contact your administrator immediately.</p>
      </div>`,
  })
}
...
async function sendOtpSms(phone, otp) {
  ...
  if (!mobile || mobile.length < 10) {
    console.error('[OTP SMS] Invalid phone after normalize:', phone, '->', mobile)
    return
  }
  // Fast2SMS expects 10-digit number (no country code)
  const digits10 = mobile.startsWith('91') && mobile.length === 12 ? mobile.slice(2) : mobile
  try {
    console.log('[OTP SMS] Fast2SMS sending to:', digits10)
    const res = await fetch('https://www.fast2sms.com/dev/bulkV2', {
      method: 'POST',
      headers: { authorization: process.env.FAST2SMS_API_KEY, 'Content-Type': 'application/json' },
      body: JSON.stringify({ route: 'otp', variables_values: otp, numbers: digits10, flash: 0 }),
    })
    const json = await res.json()
    if (!json.return) console.error('[OTP SMS] Fast2SMS error:', json.message)
    else console.log('[OTP SMS] Fast2SMS sent, id:', json.request_id)
  } catch (e) {
    console.error('[OTP SMS] fetch error:', e.message)
  }
}

exports.sendAdminOtp = onCall({ region: REGION }, async (request) => {
  const { userId, email } = request.data || {}
  if (!userId) throw new HttpsError('invalid-argument', 'userId is required')
  if (!email)  throw new HttpsError('failed-precondition', 'No email address on this admin account. Add one in HR settings.')

  const otp = String(Math.floor(100000 + Math.random() * 900000))
  const expiresAt = Timestamp.fromDate(new Date(Date.now() + 5 * 60 * 1000))

  await db.collection('adminOtps').doc(userId).set({
    otp, expiresAt, used: false, createdAt: Timestamp.now(),
  })
  ...
  try {
    ...
    throw new HttpsError('internal', `Email send failed: ${e.message}. Check Zoho SMTP credentials in GitHub Secrets.`)
  }
...
// OTP — VERIFY
// ─────────────────────────────────────────────────────────────────────────────
exports.verifyAdminOtp = onCall({ region: REGION }, async (request) => {
  const { userId, otp } = request.data || {}
  if (!userId || !otp) return { valid: false, reason: 'Missing fields' }

  const snap = await db.collection('adminOtps').doc(userId).get()
  ...
  if (data.expiresAt.toDate() < new Date()) return { valid: false, reason: 'OTP expired. Please request a new one.' }
  if (data.otp !== String(otp).trim()) return { valid: false, reason: 'Incorrect OTP.' }

  await db.collection('adminOtps').doc(userId).update({ used: true })
  return { valid: true }
})
